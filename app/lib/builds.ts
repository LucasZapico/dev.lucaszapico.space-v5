...
export const builds: Record<string, Build> = {
  spotter: {
    ...
    overview:
      "Spotter watches file changes and routes diffs to multiple LLMs in parallel — Ollama, OpenWebUI, Claude Code, or any OpenAI-compatible endpoint — then cross-validates the findings before surfacing them. The goal was a zero-interruption review loop that runs while you work, not after you push.",
    stack: ["Node.js", "TypeScript", "Chokidar", "Ollama", "Claude Code", "OpenAI API"],
    highlights: [
      {
        title: "Multi-model consensus engine",
        body: "Findings from each model are clustered using Jaccard token similarity and union-find grouping. Only findings that survive a configurable quorum make it through — reducing noise without losing signal. A 'judge' strategy lets one model (typically Claude) act as a second pass over the surviving candidates.",
      },
      {
      ...
      },
      {
        ...
        body: "Config loads from hardcoded defaults → global XDG config → project .spotter.yml → CLI flags. Environment variables interpolate inline. Per-model system prompts let you give different instructions to each backend — one watching for runtime bugs, another for structural patterns.",
      },
      {
  ...
  "investment-platform": {
    ...
    tagline: "Multi-timeframe ensemble trading system with live paper trading",
    ...
    overview:
      "A quantitative trading system built around the hypothesis that a single model can't capture both short-term noise and multi-day trends. Four XGBoost models specialise across different timeframes, each with its own feature set, profit target, and holding period. The system runs continuously on dedicated hardware — collecting live data, generating signals, and executing paper trades while iterating on model architecture. 2,668 trained models in PostgreSQL, GPU-accelerated training on an RTX 4090, and a multi-source data pipeline that has been running for months.",
    ...
    highlights: [
      ...
      },
      {
        ...
        body: "Position size is computed as Kelly criterion × volatility adjustment × drawdown scaling × confidence weighting — each layer applying a multiplicative constraint. High volatility shrinks position. Model-specific drawdown limits the risk per trade. Non-linear confidence weighting (below 40% → 40% of base; above 90% → 120%) rewards conviction without overexposure.",
      },
      {
        title: "Honest results",
        body: "ETHUSD baseline: +7.46% return, 55.63% win rate over the test period. 16 feature/holding configurations tested — none beat the baseline. The models didn't outperform buy-and-hold on the test window. The infrastructure is real, the data pipeline is real, the engineering is real. The alpha isn't there yet.",
      },
    ],
    links: [],
  },
  ...
  cortana: {
    ...
    highlights: [
      ...
      },
      {
        ...
        body: "Built-in procedural 3D (Three.js morph targets), Rive vector animation (state machine with talking/emotion inputs), VRM skeleton control (@pixiv/three-vrm), and a started Ready Player Me integration. Users can upload their own .riv or .vrm files — stored in IndexedDB, resolved via blob URL. The renderer is swapped at runtime without restarting.",
      },
      {
        title: "Streaming parser for extended thinking",
        body: "Claude's extended thinking arrives as <think>…</think> tags interleaved with the response stream, sometimes split across chunk boundaries. The parser maintains state across chunks to extract reasoning content correctly and display it separately — without buffering the full response first.",
      },
      {
